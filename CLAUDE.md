...
│   │   │       ├── settings/            # アカウント設定（メール・パスワード・プラン・解約）
...
│   │   │   ├── users/me/        # 現在のユーザー情報取得・更新
│   │   │   ├── stripe/
│   │   │   │   ├── checkout/    # Checkout セッション作成 + 完了コールバック
│   │   │   │   └── webhook/     # Stripe Webhook 受信
│   │   │   ├── domains/         # カスタムドメイン登録・削除・DNS確認
│   │   │   ├── upload/          # 画像アップロード
│   │   │   └── form/            # 公開LPフォーム受信
...
│   │   │   ├── CustomDomainPanel.tsx # カスタムドメイン設定（Pro限定）
│   │   │   ├── AddSectionModal.tsx  # セクション追加モーダル
│   │   │   └── forms/              # セクション種別ごとの編集フォーム（14種）
│   │   │       ├── HeroForm.tsx
│   │   │       ├── FeaturesForm.tsx
│   │   │       ├── TestimonialsForm.tsx
│   │   │       ├── PricingForm.tsx
│   │   │       ├── PricingTableForm.tsx
│   │   │       ├── FaqForm.tsx
│   │   │       ├── CtaForm.tsx
│   │   │       ├── StepsForm.tsx
│   │   │       ├── StatsForm.tsx
...
│   │   │   ├── NewProjectPage.tsx   # 新規プロジェクト作成ウィザード
│   │   │   ├── SettingsPage.tsx     # アカウント設定ページ
│   │   │   ├── CreateProjectModal.tsx
│   │   │   ├── Header.tsx
│   │   │   ├── ProjectList.tsx
│   │   │   └── SiteThumbnail.tsx
│   │   ├── shared/              # 複数箇所で使う共通 UI
│   │   │   └── PlanMenuSection.tsx  # アカウントメニュー内のプラン情報表示
│   │   └── providers/
...
│   │   ├── plans.ts             # プラン制限定数（PLAN_LIMITS・AIクレジット消費）
│   │   ├── stripe.ts            # Stripe クライアント初期化
│   │   ├── sectionStyle.ts      # buildSectionStyle ユーティリティ
│   │   ├── vercel.ts            # Vercel API ラッパー（カスタムドメイン管理）
...
| GET | `/api/users/me` | — | 必要 | 現在のユーザー情報取得 |
| PATCH | `/api/users/me` | updateEmailSchema / updatePasswordSchema | 必要 | メール・パスワード変更 |
| PATCH | `/api/projects/[projectId]` | updateProjectSchema | 必要 | プロジェクト名変更 |
| POST | `/api/stripe/checkout` | — | 必要 | Stripe Checkout セッション作成（Pro アップグレード） |
| GET | `/api/stripe/checkout/success` | — | 不要 | Checkout 完了後コールバック（plan を pro に更新） |
| POST | `/api/stripe/webhook` | — | 不要 | Stripe Webhook 受信（サブスク更新・解約・支払失敗） |
| POST | `/api/domains` | — | 必要 | カスタムドメイン登録（Pro 限定・Vercel API 呼び出し） |
| DELETE | `/api/domains/[domain]` | — | 必要 | カスタムドメイン削除 |
| GET | `/api/domains/[domain]/verify` | — | 必要 | DNS 確認・domainVerified を更新 |
