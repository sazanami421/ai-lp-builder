...
/dashboard/new
  ↓
[Step 0] 作成方法を選ぶ
  ├─ ① AIにおまかせ（デザイン＋コンテンツ全てAI生成）
  │     ↓
  │   [Step 1-3] ヒアリング（業種/ターゲット/強み/機能×3/料金プラン数/CTA/カラートーン）
  │     ↓
  │   [Step 4] プロジェクト名入力
  │     ↓
  │   POST /api/ai/generate → 生成中スピナー → エディター
  │
  ├─ ② テンプレ選択＋AI生成（デザインは人が選択、コンテンツはAI生成）
  │     ↓
  │   [Step 1] テンプレート選択（5種）
  │     ↓
  │   [Step 2-3] ヒアリング（カラートーン質問なし・それ以外は①と共通）
  │     ↓
  │   [Step 4] プロジェクト名入力
  │     ↓
  │   POST /api/ai/generate → 生成中スピナー → エディター
  │
  └─ ③ 手動（現状の CreateProjectModal、変更なし）
        テンプレート選択 → プロジェクト名 → エディター（空）
```

### ヒアリング項目（①と②で共通・カラートーンのみ①専用）

| # | 質問 | 形式 | ①AI全生成 | ②テンプレ＋AI |
|---|------|----|:---:|:---:|
| 1 | 業種・カテゴリ | 選択（SaaS/EC/士業/飲食/医療・美容/その他） | ○ | ○ |
| 2 | ターゲット | 選択（個人向け/中小企業向け/大企業向け/幅広く） | ○ | ○ |
| 3 | 一番の強み・USP | テキスト（1〜2行） | ○ | ○ |
| 4 | 主な特徴・機能（3つ） | テキスト × 3 | ○ | ○ |
| 5 | 料金プランの数 | 選択（なし/1/2/3） | ○ | ○ |
| 6 | CTAのゴール | 選択（無料登録/資料請求/購入/お問い合わせ） | ○ | ○ |
| 7 | カラートーン | 選択（5択）→ 内部でテンプレートにマッピング | ○ | ✕ |
| 8 | プロジェクト名 | テキスト（最後に入力） | ○ | ○ |

### カラートーン → テンプレートマッピング（①のみ・ユーザーにはテンプレート名を見せない）

| 選択肢 | 内部マッピング |
|--------|--------------|
| ミニマル（白・黒） | simple |
| ブルー（ビジネス・信頼感） | business |
| グリーン（ナチュラル・健康） | natural |
| オレンジ（ポップ・親しみやすい） | pop |
| ダーク（高級感） | premium |
