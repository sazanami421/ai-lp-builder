...
export default function TestimonialsSection({ data, styleOverrides }: Props) {
  ...
  if (variant === 'single') {
    ...
    if (!item) return null;
    return (
      <section className="py-24 px-6" style={sectionStyle}>
        <div className="mx-auto max-w-3xl text-center">
          ...
          <svg className="mx-auto mb-6 h-10 w-10 opacity-20" viewBox="0 0 24 24" fill="currentColor">
          ...
          <p className="mb-10 text-xl leading-relaxed" style={{ opacity: 0.85 }}>
            {item.body}
          </p>
    ...
    <section className="py-20 px-6" style={sectionStyle}>
      ...
      <div className="mx-auto grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {data.items.map((item, i) => (
          ...
          >
            <p className="mb-4" style={{ opacity: 0.8 }}>&ldquo;{item.body}&rdquo;</p>
